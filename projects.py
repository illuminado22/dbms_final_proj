...
def ensure_projects_schema():
    try:
        ...
        cur.execute("SHOW COLUMNS FROM projects LIKE 'description'")
        if not cur.fetchone():
            cur.execute("ALTER TABLE projects ADD COLUMN description TEXT NULL")
        cur.execute("SHOW COLUMNS FROM projects LIKE 'year_completed'")
        if not cur.fetchone():
            cur.execute("ALTER TABLE projects ADD COLUMN year_completed INT NULL")
        cur.execute("SHOW COLUMNS FROM projects LIKE 'team_id'")
        if not cur.fetchone():
            cur.execute("ALTER TABLE projects ADD COLUMN team_id INT NULL")
    ...
    def _delete(self, pid):
        if confirm_delete(self, "Delete this project?"):
            try:
                ...
                conn.commit(); conn.close()
                reset_auto_increment('projects', 'project_id', get_conn)
                self._load()
